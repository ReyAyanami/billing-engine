import { CommandHandler, EventBus, ICommandHandler } from '@nestjs/cqrs';
import { Inject, Logger } from '@nestjs/common';
import { CompensateTransactionCommand } from '../commands/compensate-transaction.command';
import { TransactionAggregate } from '../aggregates/transaction.aggregate';
import type { IEventStore } from '../../../cqrs/interfaces/event-store.interface';

/**
 * Handler for CompensateTransactionCommand.
 * Compensates (rolls back) a transaction by reverting changes made during a saga.
 */
@CommandHandler(CompensateTransactionCommand)
export class CompensateTransactionHandler implements ICommandHandler<CompensateTransactionCommand> {
  private readonly logger = new Logger(CompensateTransactionHandler.name);

  constructor(
    @Inject('EVENT_STORE') private eventStore: IEventStore,
    private eventBus: EventBus,
  ) {}

  async execute(command: CompensateTransactionCommand): Promise<void> {
    this.logger.log(
      `[CompensateTransactionHandler] Compensating transaction: ${command.transactionId}`,
    );
    this.logger.log(`   Reason: ${command.reason}`);
    this.logger.log(
      `   Compensation actions: ${command.compensationActions.length}`,
    );

    try {
      // Load transaction aggregate from event history
      const events = await this.eventStore.getEvents(
        'Transaction',
        command.transactionId,
      );

      if (events.length === 0) {
        throw new Error(`Transaction not found: ${command.transactionId}`);
      }

      // Reconstruct aggregate
      const transaction = TransactionAggregate.fromEvents(events);

      // Compensate the transaction
      transaction.compensate({
        reason: command.reason,
        compensationActions: command.compensationActions,
        correlationId: command.correlationId,
        causationId: command.commandId,
        metadata: {
          actorId: command.actorId,
        },
      });

      // Get and persist uncommitted events
      const newEvents = transaction.getUncommittedEvents();
      await this.eventStore.append(
        'Transaction',
        command.transactionId,
        newEvents,
      );

      // Publish events
      newEvents.forEach((event) => {
        this.eventBus.publish(event);
      });

      transaction.commit();

      this.logger.log(
        `✅ [CompensateTransactionHandler] Transaction compensated: ${command.transactionId}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ [CompensateTransactionHandler] Failed to compensate transaction ${command.transactionId}`,
        error,
      );
      throw error;
    }
  }
}
