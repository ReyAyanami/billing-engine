  ...
  implements IQueryHandler<GetTransactionsByAccountQuery>
{
  ...
  async execute(query: GetTransactionsByAccountQuery): Promise<TransactionProjection[]> {
    this.logger.log(`Executing GetTransactionsByAccountQuery for: ${query.accountId}`);

    const projections = await this.projectionService.findByAccount(query.accountId);

    this.logger.log(`Found ${projections.length} transaction(s) for account: ${query.accountId}`);
