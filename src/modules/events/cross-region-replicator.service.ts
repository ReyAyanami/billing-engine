...
export class CrossRegionReplicatorService implements OnModuleInit {
  ...
  constructor(
    ...
    this.remoteRegions = this.configService
      .get<string>('REMOTE_REGIONS', '')
      .split(',')
      .filter((r) => r !== '');
  }
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      `Starting replication from regions: ${this.remoteRegions.join(', ')}`,
    );
  ...
  private async startReplicationLoop() {
    ...
    const topics = ['billing.account.events', 'billing.transaction.events'];
      ...
      eachMessage: async ({ topic, message }: EachMessagePayload) => {
        try {
          ...
          if (!parseResult.success) {
            this.logger.warn(
              `Failed to parse event for replication from topic ${topic}: ${parseResult.error}`,
            );
            return;
          }
          ...
          this.logger.debug(
            `Replicating event ${event.eventId} [${event.eventType}] from region ${event.regionId}`,
          );
          ...
          await this.eventStore.append(event.aggregateType, event.aggregateId, [
            event as unknown as DomainEvent,
          ]);

          this.logger.log(
            `✅ Replicated ${event.eventType} [${event.eventId}] from ${event.regionId} to local store`,
          );
        } catch (error) {
          this.logger.error(
            `Error replicating message from topic ${topic}:`,
            error,
          );
        }
      },
